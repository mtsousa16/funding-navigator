...
export function PostCard({ post, currentUserId }: PostCardProps) {
  ...
  useEffect(() => {
    supabase.from('profiles').select('display_name, avatar_url').eq('user_id', post.user_id).single()
  ...
  const loadComments = async () => {
    ...
    if (data) {
      const withProfiles = await Promise.all(data.map(async c => {
        const { data: p } = await supabase.from('profiles').select('display_name').eq('user_id', c.user_id).single();
        return { ...c, display_name: p?.display_name || 'Usuário' };
      }));
  ...
  return (
    <div className="bg-card border-b border-border">
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <span className="text-sm font-semibold text-foreground">{profile?.display_name || 'Usuário'}</span>
      ...
      {post.content && (
        <p className="px-3 pb-2 text-sm">
          <span className="font-semibold mr-1">{profile?.display_name || 'Usuário'}</span>
          {post.content}
        </p>
      )}
